...
export function FilterBar({
  ...
  return (
    <div className="px-3 py-2 border-b border-[#1e3a5f] space-y-1.5 flex-shrink-0">
      {/* テキスト検索 */}
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-8 pr-3 py-1 bg-[#1e293b] border border-[#334155] rounded-md text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-[#3b82f6] transition-colors"
        />
      </div>

      {/* 学校種別 */}
      <div className="flex items-center gap-1.5">
        <span className="text-slate-500 text-[11px] w-12 flex-shrink-0">種別</span>
        {TYPES.map((t) => (
          <button
            key={t}
      ...
      {/* 区分 */}
      <div className="flex items-center gap-1.5">
        <span className="text-slate-500 text-[11px] w-12 flex-shrink-0">区分</span>
        {CATEGORIES.map((c) => (
          <button
            key={c}
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="text-slate-500 text-[11px] w-12 flex-shrink-0">フェーズ</span>
        {STAGES.map((s) => (
          <button
            key={s}
        ...
        {hasFilter && (
          <button
            onClick={onReset}
            className="px-2 py-0.5 text-[11px] rounded border border-slate-600 text-slate-500 hover:text-slate-300 hover:border-slate-400 transition-colors ml-auto"
          >
            リセット
          </button>
        )}
